...
embedToBullet :: Monad m => BulletT s m a -> DanmakuT b (ReaderT s m) a -> BulletT s (WriterT [b] m) a
...
flatten :: Monad m => DanmakuT (BulletT s (WriterT [b] m) c) m a -> DanmakuT (Either s b) m a
flatten = flatten' [] where
    flatten' tamas danmaku = do
        ...
        either (flatten' $ concat tss ++ ts) return cont
