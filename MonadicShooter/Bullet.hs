module MonadicShooter.Bullet where
import MonadicShooter.Graphic
import MonadicShooter.Field
import Control.Monad.Trans.State
import GHC.Float
import Data.Vect
import Data.Danmaku
import Graphics.Gloss.Data.Picture
import Graphics.Gloss.Geometry.Angle
import qualified Data.Map as Map

data RealBullet = RealBullet Vec2 Vec2 String | Gone

updateBullet :: RealBullet -> RealBullet
updateBullet (RealBullet v p@(Vec2 x y) c)
    | x < leftBound || x > rightBound || y > upperBound || y < lowerBound = Gone
    | otherwise = RealBullet v (p &+ v) c
updateBullet Gone = Gone

isActive Gone = False
isActive _ = True

drawBullet :: Map.Map String Picture -> RealBullet -> Picture
drawBullet m (RealBullet v (Vec2 x y) c) = Translate x y
    $ Rotate (negate $ radToDeg $ angle2 v - pi / 2)
    $ m Map.! c